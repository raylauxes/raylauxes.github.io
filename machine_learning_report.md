...
### ML_15_03_主成分分析
```
どうなるべく情報量を残すか？それは分散の大きさで情報量を確認
```
![Image](/ML_15_03_主成分分析_00m00s.png)
> aが変わると、黄色線の角度も変わる。aがscalarのよう?


### ML_15_04_主成分分析
```
ノルムが違っても、向きが同じなベクトルを同じものと考えられるため、「ノルムが1」という制約条件を加える
```
![Image](/ML_15_04_主成分分析_00m00s.png)
> Q:ラグランジュ関数は？


### ML_15_05_主成分分析
```
結局、最適解を求めること = 固有値と固有ベクトルを求めること
```
![Image](/ML_15_05_主成分分析_00m00s.png)
> 


### ML_15_06_主成分分析
```
寄与率（きよりつ）：成分はどれぐらいの情報量を持っているか。
主成分分析などデータ圧縮の手法はディープラーニングにも用いられる。例えば、1000次元のデータがあっても、10次元で90%の情報量を残せれば、1000次元を10次元に圧縮する。
```
![Image](/ML_15_06_主成分分析_00m00s.png)


### ML_16_01_ハンズオン（乳がんデータ分析）
参考記事：[scikit-learnで乳がんデータセットを主成分分析する](https://ohke.hateblo.jp/entry/2017/08/11/230000)
```
目的変数は2列目の「diagnosis」（診断結果 (良性がB / 悪性がM)）
説明変数は3列以降
```
> DataFrameの「loc」メソッドで「radius_mean」以降の列を抽出！
![Image](/skl_pca.ipynb_cell_27_loc.png)

> 「sklearn.decomposition.PCA.explained_variance_ratio」で各成分はどれぐらいの情報量を持っているのかを確認。下記の図のように、第一成分の情報量が43%であり、第二成分の情報量が19％など
![Image](/skl_pca.ipynb_cell_15_PCA.explained_variance_ratio.png)

> 実演リンク：[skl_pca.ipynb](https://drive.google.com/file/d/15e3D7GJ3vxkbpN9eWOSlAL0uZAJgyRgT/view?usp=sharing)
...
kを決めないといけない
```
![Image](/ML_17_01_k近傍法_00m55s.png)
> k平均法：the k-nearest neighbors algorithm (k-NN)
...
### ML_18_01_k-means
```
k-meansはクラスタリング（似たものをグループ化）手法の１つ。
クラスターの数（k）を事前に決めて、各クラスタの中心値の初期値を適当にし、安定するまで繰り返す

k平均法のアルゴリズム：
1、各クラスタ中心の初期値を設定
2、各データを距離が最も近いクラスタを割り当てる
3、各クラスタの中心（平均ベクトル）を計算
4、収束するまで2と3を繰り返す
```
![Image](/ML_18_01_k-means_02m10s.png)


### ML_18_02_k-means
```
k-平均法に注意すべきもの：
1、初期値が近いと、データをうまく分類できない場合がある
2、kの値を変えると、分類の結果も変わる
```
![Image](/ML_18_02_k-means_01m17s.png)
![Image](/ML_18_02_k-means_01m59s.png)
